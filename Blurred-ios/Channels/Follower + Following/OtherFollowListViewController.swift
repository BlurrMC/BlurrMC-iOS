...
class OtherFollowListViewController: UIViewController, UITableViewDataSource, UITableViewDataSourcePrefetching {
    ...
    func PreFetch(success: @escaping (_ response: AFDataResponse<Any>?) -> Void, failure: @escaping (_ error: NSError?) -> Void) {
        ...
        AF.request("https://blurrmc.com/api/v1/channelsfollowing/\(followingVar)", method: .get, parameters: parameters, headers: headers).responseJSON { response in
    ...
    func downloadJson() { 
        let Id = followingVar
        let url = URL(string: "https://blurrmc.com/api/v1/channelsfollowing/\(Id).json")

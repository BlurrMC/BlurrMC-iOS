...
class CommentingViewController: UIViewController, UITextFieldDelegate {
    ...
    func getAvatar() {
        ...
        AF.request("https://blurrmc.com/api/v1/channels/\(userId).json").responseJSON { response in
            var JSON: [String: Any]?
            do {
                JSON = try JSONSerialization.jsonObject(with: response.data!, options: []) as? [String: Any]
                guard let imageUrl = JSON!["avatar_url"] as? String else { return }
                guard let railsUrl = URL(string: "https://blurrmc.com\(imageUrl)") else { return }
                DispatchQueue.main.async {
    ...
    func submitComment(reply: Bool) {
        ...
        let myUrl = URL(string: "https://blurrmc.com/api/v1/videos/\(videoId)/comments/")
    ...
    func downloadJson(fromReply: Bool) {
        ...
        AF.request("https://blurrmc.com/api/v1/videos/\(videoId)/comments/", method: .get, headers: headers).responseJSON { response in
            ...
            do {
                ...
                print("error code: f0bm39fna9319g427hf42, \(error)")
                return
            }
        }
    }
...
extension CommentingViewController: CommentCellDelegate {
    ...
    func submitEditedComment(indexPath: IndexPath, reply: Bool) {
        ...
        case true:
            ...
            let url = String("https://blurrmc.com/api/v1/videos/\(videoId)/comments/\(reply.id)")
        ...
        case false:
            ...
            let url = String("https://blurrmc.com/api/v1/videos/\(videoId)/comments/\(comment.id)")
            AF.request(URL.init(string: url)!, method: .patch, parameters: params as Parameters, encoding: JSONEncoding.default, headers: headers).responseJSON { (response) in
                var JSON: [String: Any]?
                do {
                    JSON = try JSONSerialization.jsonObject(with: response.data!, options: []) as? [String: Any]
    ...
    func deleteComment(indexPath: IndexPath, commentId: String?, reply: Bool) {
        ...
        let url = String("https://blurrmc.com/api/v1/videos/\(videoId)/comments/\(commentId)")
    ...
    func sendDeleteLikeRequest(likeid: Any?, commentId: String, cell: Comment, indexPath: IndexPath, reply: Bool) {
        ...
        if likeid != nil, let likeid = likeid {
            let url = "https://blurrmc.com/api/v1/videos/\(videoId)/comments/\(commentId)/commentlikes/\(likeid)"
            throttler.throttle( {
                AF.request(URL.init(string: url)!, method: .delete, encoding: JSONEncoding.default, headers: headers).responseJSON { (response) in
                    var JSON: [String: Any]?
    ...
    func sendLikeRequest(commentId: String, cell: Comment, indexPath: IndexPath, reply: Bool) {
        ...
        let url = String("https://blurrmc.com/api/v1/videos/\(videoId)/comments/\(commentId)/commentlikes")
        let throttler = Throttler(minimumDelay: 5)
        throttler.throttle( {
...
extension CommentingViewController: UITableViewDataSourcePrefetching {
    ...
    func PreFetch(success: @escaping (_ response: AFDataResponse<Any>?) -> Void, failure: @escaping (_ error: NSError?) -> Void) {
        ...
        AF.request("https://blurrmc.com/api/v1/videos/\(videoId)/comments/", method: .get, parameters: parameters, headers: headers).responseJSON { response in
